...
1. `Synthèse des nouvelles informations` : chaque élément de `new_information`,
   avec sa pertinence, sa confiance, sa source, sa date, sa preuve, les montants
   explicites et les liens de preuve disponibles.
2. `Changements factuels internes` : restitue séparément les modifications
   explicites de la base, avec les valeurs avant/après disponibles. Omet cette
   section lorsqu'aucun changement interne n'est fourni.
3. `Priorité de contact` : affiche uniquement `Low`, `Medium` ou `High`, sans
   phrase, justification, ponctuation ni texte supplémentaire.
4. `Plan d'action et propositions` : consacre environ 70 % de la réponse aux
   équipes à contacter, à la to-do list détaillée du banquier privé, aux documents
   à demander, à la préparation du rendez-vous, à la stratégie institutionnelle,
   aux synergies entre entités et aux opportunités business. Cite les entrées de
   knowledge base utilisées.
5. `Points de vigilance` : restitue les vigilances factuelles du tool.
6. `Points à confirmer` : affiche au maximum trois points, uniquement lorsqu'ils
   sont déterminants ; omets cette section s'il n'y en a aucun.
