...
object. The latter is intentionally optional until the Société Générale offer
catalogue is available. Prompts live in `proposition/prompts.py`, so that the
future expert prompt can replace the initial version without changing tool wiring.
